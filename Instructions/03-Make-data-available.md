...
Si userà uno script shell che usa l'interfaccia della riga di comando di Azure per effettuare il provisioning dell'area di lavoro e delle risorse necessarie. Si userà quindi il Designer nel studio di Azure Machine Learning per eseguire il training e il confronto dei modelli.
...
1. Copiare il nome dell'account di archiviazione dalla parte superiore della pagina. Il nome deve iniziare con **mlwdp100storage...** Sarà necessario incollare questo valore anche nel notebook.

> **Nota**: copiare la chiave e il nome dell'account in un Blocco note per evitare l'uso automatico delle maiuscole e minuscole (che si verifica in Word). La chiave fa distinzione tra maiuscole e minuscole.
