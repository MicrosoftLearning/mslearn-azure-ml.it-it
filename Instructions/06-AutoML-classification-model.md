...
Verrà creato un nuovo processo nell'area di lavoro di Azure Machine Learning. Il processo tiene traccia degli input definiti nella configurazione del processo, l'asset di dati usato e gli output come le metriche per valutare i modelli.
...
3. Al termine del processo Automate Machine Learning, esplorare i dettagli del processo nello studio:
    - La scheda **Data guardrails** mostra se i dati di training hanno avuto problemi.
    - La scheda **Modelli** mostrerà tutti i modelli sottoposti a training.
    - Selezionare **Visualizza spiegazione** per il modello migliore per comprendere quali funzionalità hanno influenzato il valore di destinazione più.
