...
1. Rimuovere tutte le estensioni dell'interfaccia della riga di comando di Machine Learning (sia versione 1 che 2) per evitare conflitti con le versioni precedenti con questo comando:
...
1. Attendere la creazione dell'area di lavoro e delle relative risorse associate. Questa operazione richiede in genere circa 5 minuti. 
...
Quando i data scientist sviluppano un modello di Machine Learning nell'area di lavoro di Azure Machine Learning, vogliono usare una macchina virtuale in cui possono eseguire notebook di Jupyter. Per lo sviluppo, **un'istanza di calcolo** è una soluzione ideale. 
...
## Configurare la stazione di lavoro con il studio di Azure Machine Learning

Anche se l'interfaccia della riga di comando di Azure è ideale per l'automazione, è possibile esaminare l'output dei comandi eseguiti. È possibile usare Azure Machine Learning Studio per verificare se sono state create risorse e asset e verificare se i processi sono stati eseguiti correttamente o verificare il motivo per cui un processo non è riuscito. 
...
Dopo aver verificato che il calcolo necessario è stato creato, è possibile usare Python SDK per eseguire uno script di training. Si installerà e si userà Python SDK nell'istanza di calcolo ed si eseguirà il training del modello di Machine Learning nel cluster di calcolo.
...
1. Verificare che il notebook usi il kernel **Python 3.8 - AzureML** . Ogni kernel ha una propria immagine con il proprio set di pacchetti preinstallati.
...
1. Viene elencato un nuovo esperimento denominato **diabetes-training**. Selezionare l'ultimo processo **diabetes-pythonv2-train**.
1. Esaminare **le proprietà** del processo. Si noti lo **stato** del processo: 
    - **Accodato**: il processo è in attesa che il calcolo diventi disponibile.
...
1. Al termine del processo, è possibile trovare *l'AUC* e *l'accuratezza* del modello in **Metriche**. 
1. In **Output e log** sono disponibili:
    - File pickle del modello **nell'output** della cartella. 
    - Output dello script in **user_logs/std_log.txt**. L'output delle istruzioni **di stampa** nello script verrà visualizzato qui. Se si verifica un errore a causa di un problema con lo script, verrà visualizzato anche il messaggio di errore.
